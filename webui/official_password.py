...
def official_set_account_password(
    ...
    try:
        ...
        logger.warning(f"[official-pwd] 邮箱 Provider 初始化异常: {e}")
        mail_provider = create_mail_provider("cf_temp", settings)

    logger.info(f"[official-pwd] 正在为 {email_clean} 启动官方全自动设置密码流程 (Provider: {mail_provider.display_name})...")
    ...
    logger.info(f"[official-pwd] 官方重置密码验证码已发送，正在等待从 {mail_provider.display_name} 收件 (超时 {timeout}s)...")
    ...
    logger.info(f"[official-pwd] 成功获取到新验证码: {otp_code}，正在校验...")
